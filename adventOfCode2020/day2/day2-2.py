...
for pssw in passwords:

  password = pssw.split(':')[1]
  letter = pssw.split(':')[0].split(' ')[1]
  startPos = int(pssw.split(':')[0].split(' ')[0].split('-')[0])
  endPos = int(pssw.split(':')[0].split(' ')[0].split('-')[1])

  string_array = np.array(list(password))
  char_indexes = np.where(string_array == letter)
