...
def validate_data(hcl, ecl, iyr, eyr, pid, byr, hgt):

  validPassport = True
  
  if not bool(re.match("^(#{1})([0-9A-F]{8}|[0-9A-F]{6})$", hcl.upper())):
    print('validate')
    print('hcl: ', hcl)
    validPassport = False
  ...
  if 'in' in hgt and (not hgt.split('in')[0].isdigit() or int(hgt.split('in')[0]) < 59 or int(hgt.split('in')[0]) > 76):
    print('validate')
    print('hgt: ', hgt)
    validPassport = False
  elif 'cm' in hgt and (not hgt.split('cm')[0].isdigit() or int(hgt.split('cm')[0]) < 150 or int(hgt.split('cm')[0]) > 193):
    print('validate')
    print('hgt: ', hgt)
    validPassport = False
  elif not 'in' in hgt and not 'cm' in hgt:
    print('validate')
    print('hgt: ', hgt)
    validPassport = False
  
  print("validation: ", validPassport)
...
for index, passportLine in enumerate(passports):
  ...
  if passportLine == "":
    ...
    print('print before validate: ', hcl, ecl, iyr, eyr, pid, byr, hgt)
    if validate_data(hcl, ecl, iyr, eyr, pid, byr, hgt):
      validPassports += 1
    
    # if ecl and hcl and iyr and eyr and pid and byr and hgt:
    #   validPassports += 1
    ...
    passportInfo = passportLine.split(' ')
    print(passportInfo)
    for data in passportInfo:
      if "hcl" in data:
        print(data)
        hcl = data.split(':')[1]
        print(hcl)
