...
for index, passportLine in enumerate(passports):
  
  if passportLine == "":
    
    numPassports += 1
    
    if hasEcl and hasHcl and hasIyr and hasEyr and hasPid and hasByr and hasHgt:
      validPassports += 1
    ...
    if "hcl" in passportLine:
      data = passportLine.split(' ')
      for d in data:
        if "hcl" in d:
          if re.match("^(#{1})([0-9A-F]{8}|[0-9A-F]{6})$", d.split(':')[1].upper()):
            hasHcl = True
        

    if "ecl" in passportLine:
      data = passportLine.split(' ')
      for d in data:
        if "ecl" in d:
          if d.split(':')[1] in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth']:
            hasEcl = True
    
    if "iyr" in passportLine:
      data = passportLine.split(' ')
      for d in data:
        if "iyr" in d:
          if d.split(':')[1].isdigit() and int(d.split(':')[1]) >= 2010 and int(d.split(':')[1]) <= 2020:
            hasIyr = True
    
    if "eyr" in passportLine:
      data = passportLine.split(' ')
      for d in data:
        if "eyr" in d:
          if d.split(':')[1].isdigit() and int(d.split(':')[1]) >= 2020 and int(d.split(':')[1]) <= 2030:
            hasEyr = True    
    ...
    if "byr" in passportLine:
      ...
      for d in data:
        if "byr" in d:
          if d.split(':')[1].isdigit() and int(d.split(':')[1]) >= 1920 and int(d.split(':')[1]) <= 2002:
            hasByr = True

    if "pid" in passportLine:
      data = passportLine.split(' ')
      for d in data:
        if "pid" in d:
          if d.split(':')[1].isdigit() and len(d.split(':')[1]) == 9:
            hasPid = True
    
    if "hgt" in passportLine:
      data = passportLine.split(' ')
      for d in data:
        if "hgt" in d:
          hgt = d.split(':')[1]
          if 'in' in hgt:
            if hgt.split('in')[0].isdigit() and int(hgt.split('in')[0]) >= 59 and int(hgt.split('in')[0]) <= 76:
              print(d.split(':')[1])
              hasHgt = True
          elif 'cm' in hgt:
            if hgt.split('cm')[0].isdigit() and int(hgt.split('cm')[0]) >= 150 and int(hgt.split('cm')[0]) <= 193:
              print(d.split(':')[1])
              hasHgt = True

  if (len(passports) -1 == index):
    numPassports += 1
...
print("num passports: ", numPassports)
